<template>
  <div class="col-md-4">
    <p style="color:white;padding:5px 10px;border-bottom:1px solid #676565;margin:2px 0px">内存利用率Top5</p>
    <div class="king-block king-block-themed bk-ranger-box mb10" id="disk_sum">
      <div class="ranger-wrapper">
        <table class="mb0 pr15 ranger-header-box color">
          ...
          <tbody id>
            <tr v-for="(item,key) in list" :key="key" v-if="key<5" >
              ...
              <td style="width:20%;color:#03a9f4">{{item.value | value}}%</td>
            </tr>
          </tbody>
        </table>
      </div>
...
export default {
  data() {
    return {
      ...
      original:false,   //默认从大到大排列
    };
  },
  filters:{
    value:function(val){
        val =parseInt(val.slice(0,4)) 
        val = 100- val
        return val
    },
    name:function(value){
  ...
  methods: {
    Getdata:function(){
      var _this = this
      _this.goodsList = []
      _this.$http.ZabbixServerAll(16).then(function(data){
        data.result.forEach(function(element) {
          _this.$http.ZabbixGetMemKey(element.hostid).then(function(data1){
            _this.$http.ZabbixGetKeyValue(data1[0].itemid).then(function(data){
              element["value"] = data[0].value_avg
